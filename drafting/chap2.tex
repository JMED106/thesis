\chapter{\bf \sffamily Synchrony-induced modes of oscillation of a neural field model}
...
\textit{This work has been developed in collaboration with Daniele
  ...
  Montbrió, and it has been published \citep*{Esnaola-Acebes2017b} % in Physical Review E.   Jose
...
waves with a specific temporal frequency, analogously to those in a tense string. 
...
Since the pioneering work of \citet{Wilson1973}, \citet{Ama74,Amari1977},
and \citet{Nunez1974}, continuum descriptions of neuronal activity 
have become a powerful modeling  tool in neuroscience
\citep{Erm98,Coombes2005,ET10,Bre12,CGP14,Deco2008}. 
...
can lead to oscillations~\citep{Wilson1972,Erm94,Brunel2003a}. 
...
firing rate models~\citep{Erm94}.
...
~\citep{Amari1977,Pinto2001}.  Similar patterns can also be found in NFMs with   
spatially dependent delays ---modeling the effect of the finite velocity 
propagation of action potentials~\citep{Wilson1973,Jirsa1997}---
%with heterogeneous synaptic connections, and also in stochastic spiking networks
%with a mean field coupling 
as a great deal of theoretical work indicates
~\citep[see~e.g.][]{Coombes2003,Atay2004,Coombes2009,Zhang2007,Hutt2008,Touboul2012,Vel13,Dijkstra2015}. 
%AS15,AW17

In some cases the spatio-temporal dynamics of NFMs has been directly compared 
to that observed in analogous networks of spiking neurons~\citep{Roxin2005,BBH07,Roxin2011}.
In this work it was found that non-space-dependent delays predict the existence 
of many of the spatiotemporal patterns observed in asynchronous 
networks of spiking neurons with nonlocal, space-dependent interactions.
...
high degree of spike synchronization \citep[see~e.g][]{SOA13}.


Here we report a spatiotemporal dynamical feature of 
heterogeneous networks of spiking neurons with 
nonlocal interactions that, to the best of our knowledge, have been so far 
...
time scales between excitation and inhibition or to the presence of any 
...
of a globally coupled heterogeneous population of quadratic integrate-and-fire (QIF) neurons~\citep{Montbrio2015}. This method, based on the so-called 
Ott-Antonsen theory~\citep{Ott2008,Ott2009,Ott2011}, leads to an exact macroscopic 
...
Here we extend the local firing rate model in~\citep{Montbrio2015}, to include nonlocal, 
...
by means of both a linear and nonlinear stability analysis of the 
...
\caption[E-I ring network schematics.]{Schematic representation of the ring network and 
coupling architecture under study. Panel (a) shows $N$
excitatory (red triangles) and $N$ inhibitory (blue circles) neurons
arranged on a ring. The location of  
neurons is parameterized by the angular variable 
$\phi_j = \frac{2 \pi j}{N}-\pi$, $j = 1, \dots, N$. Red (solid) and
blue (dashed) lines indicate synaptic connections between neuron pairs
$(\phi_j, \phi_k)$.  An example of the excitatory and 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Synchrony-induced modes of oscillation in Networks of QIF Neurons}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The synaptic projections of the $j$th excitatory and inhibitory neurons (located  
...
and inhibitory ($i$) neurons is determined by the (dimensionless) membrane 
...
regimes depending  on the (dimensionless) input current $I^{e,i}_j$.
...
excitability threshold, $v_j^{e,i}(0)>\sqrt{-I_j^{e,i}}$,  lead to an
unbounded growth of the membrane potential. Specifically, if $v_j^{e,i}(0) \gg
\sqrt{I_j^{e,i}}$,  then the membrane potential reaches infinity approximately 
...
$v_r=-v_p$ after a refractory period $2\tau/v_p$.
On the other hand, if $I_j^{e,i}>0$, then the neuron is in the oscillatory regime and
...
from neuron to neuron. The terms $P^{e,i}(t)$ are time-varying common inputs, 
...
and $t_k^l$ is the time of the $l$th spike of the
excitatory/inhibitory $k$th neuron. Positive and negative signs
correspond to $S_j^e$ and to $S_j^i$, respectively.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[htb]
\centerline{\includegraphics[width=\textwidth]{figs/Figure2.eps}}
\caption[Transient synchrony in ring networks of QIF neurons]{\label{Figure2} 
Transient episodes of spike synchrony in heterogeneous ring networks of 
$N =2.5\cdot 10^5$ excitatory and $N =2.5\cdot 10^5$ inhibitory QIF neurons, 
Eqs.~\eqref{qif1} and \eqref{qif2}, as a result of spatially inhomogeneous 
perturbations applied  
at time $t=0.05$~s. In Panels (a) and (b) only excitatory neurons are perturbed. In
panels (c) and (d) all neurons are perturbed. In panels (a) and (c) the 
perturbation has wave number $K=1$; in panels (b) and (d) the
perturbation has wave number $K=3$. 
...
Eqs.~\eqref{qif1} and \eqref{qif2} for a network of 
...
is applied either to only excitatory neurons (panels a, b) or to both excitatory
and inhibitory neurons. 
The left and right panels show perturbations 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Neural Field model for quadratic integrate and fire neurons}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


In the following, we aim to investigate the nature and origin of the 
spatiotemporal patterns  shown in Figure~\ref{Figure2}. 
To analyze them, we derive the NFM corresponding to the 
thermodynamic ($N\to \infty$) and continuum limits of the network 
of QIF neurons [Eqs.~\eqref{qif1} and \eqref{qif2}]. 
In additon we also take the 
limit $v_p \to \infty$, so that the QIF model~\eqref{qif1} 
is equivalent to the so-called theta-neuron model~\citep{Ermentrout1986,Izhikevich2008}. 
This leads to an exact neural field model for a network of QIF neurons (QIF-NFM).
%%%%%%%%%%%%%%%%%%
The detailed derivation is performed in Appendix
\ref{app:deriv-qif-neur}, and closely
follows that of~\citet{Montbrio2015}. 
...
single-cell properties. This scenario is schematically represented in
Figs.~\ref{Figure1}(c) and \ref{Figure1}(d). If we set
$P^e(\phi,t)=P^i(\phi,t)=P(\phi,t)$, and restrict our attention to
solutions of Eqs.~\eqref{freG} satisfying
$R^e(\phi,t)=R^i(\phi,t)\equiv R(\phi,t)$ and
$V^e(\phi,t)=V^i(\phi,t)\equiv V(\phi,t)$, then we obtain an effective
QIF-NFM in variables $R$ and $V$,
%
\begin{subequations}
\label{fre}
\begin{eqnarray}
...
Eq.~\eqref{J} as $J_K=J_K^e-J_K^i$, with $~K=0,1,\dots$, see
Fig.\ref{Figure1}(d). Solutions to Eqs.~\eqref{freG} need not satisfy
the condition $R^e=R^i$ and $V^e=V^i$, but the reduced system
Eqs.~\eqref{fre} captures the mechanism behind the oscillatory behavior
observed in the model.
Note that, in Figs.~\ref{Figure2}(a) and \ref{Figure2}(b), we
perturbed the (SHS) of the system [Eqs.~\eqref{qif1} and \eqref{qif2}]
using a current pulse to all  excitatory neurons.  
...
reduced manifold defined by Eqs.~\eqref{fre} and \eqref{mf}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{SHS and their stability:
...
analysis of both the complete model [Eqs.~\eqref{freG}], 
...
\caption[Stability of SHS ($J_K$ vs. $\bar \eta$).]{(a) Phase diagram of
  Eqs.~\eqref{fre} (with $J_0=0$)  
showing the regions of stability of the SHS, 
determined by the eigenvalues Eq.~\eqref{lambda}.  
Spatial perturbations of wave number $K>0$ show oscillatory and nonoscillatory decay to the 
...
grow and lead to a bump state (BS) with $K$ bumps (see Fig.~\ref{Figure4}). }
\label{Figure3}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
single population of neurons~\citep{Montbrio2015}. However, the stability of the 
...
gives a countably infinite set
% an infinite set 
of eigenvalues associated to the stability of perturbations 
with wave number\footnote{
...
the detailed linear stability analysis of the QIF-NFM
Eqs.~\eqref{freG}.} $K$. 
%%%%
\begin{equation}
...
indicating nonoscillatory or oscillatory dynamics of the evolution of 
perturbations of wave number $K$, respectively. 
...
\end{equation}
%
separating the parameter space into regions where standing waves of
wave number $K$
...
of wave number $K$ produce standing waves of neural activity of frequency 
$\nu_K$. 
...
Indeed, in Fig.~\ref{Figure2}(d), a perturbation of wave number $K=3$ produced standing waves, since $J_3$ was negative. The frequency of these oscillations was
...
and this process does not depend on the oscillation mode being excited. 
Finally, in Figs.~\ref{Figure3}(b) and \ref{Figure3}(c) we show 
numerical simulations of the QIF-NFM [Eq.~\eqref{fre}] using the same parameters as 
those of Fig.~\ref{Figure2}(c) and \ref{Figure2}(d), and the agreement is good. 
...
\caption[Phase diagram of the QIF-NFM ($J_K$ vs. $\bar \eta$).]{(a) Phase diagram of the 
QIF-NFM [Eqs.~\eqref{fre}] with $J_2=7.5$, $J_3=-2.5$, $J_K=0$ 
for $K>3$, and $\Delta=1$. Solid line: Supercritical [red (gray)] and 
...
Solid/dashed black lines: stable/unstable SHS.
Solid/dashed blue lines: stable/unstable bump states (BS). }
\label{Figure4}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
solid line in Figs.~\ref{Figure3}(a) and \ref{Figure4}(a). 
...
Moreover, we computed numerically a bifurcation diagram of the NFM,
  using the spectral method developed in \citep{Rankin2014} and
  available with \citep{A16}. The results, presented in
  Fig.~\ref{Figure4}(c) confirm that the unstable BS  bifurcates subcritically for
  the SHS. The unstable BS then meets a stable BS ---solid blue 
  (light gray) line--- at a fold bifurcation.   
...
near the subcritical Turing bifurcation of wave number $K=1$.
...
the other is very close to zero and positive, indicating that the SHS
is unstable.
...
\caption[Spectrum and FR profiles of BS.]{Spectrum [(a) and (b)] and firing rate 
profiles [(d) and (c)] of an unstable [(a) and (c)] and
stable [(b) and (d)] bump states of the QIF-NFM [Eqs.~\eqref{fre}].
In panels (a) and (b) the eigenvalues Eq.~\eqref{lambda} are
superimposed with red crosses.
Panel (e) shows a numerical simulation of the BS of panel (d). At 
$t=0.05$~s, a perturbation of wave number $K=6$ is applied. 
Parameters are
$J_0=0$, $J_1=10$, $J_2=7.5$, $J_3=-2.5$, $J_K=0$ for $K>3$, $\Delta=1$,
$\tau=20$~ms. Panels (a) and (c): $\bar \eta=2.2120$; panels (b), (d)
and (e): $\bar \eta=2.1828$.}
\label{Figure5}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
values of $R_*$ ---around $\phi=0$, in Figs.~\ref{Figure2}(d) and \ref{Figure2}(e)--- oscillate 
at high frequencies and these oscillations slow down as $\phi \to\pm \pi$.
...
and are not captured by traditional NFMs.

To investigate these oscillation modes we derived a novel 
NFM for QIF neurons [Eqs.~\eqref{freG}]. Alternately, and invoking the
Ott-Antonsen theory for populations of pulse-coupled  
theta neurons~\citep{LBS13,SLB14}, 
\citet{Laing2014} recently derived a NFM that is equivalent to the
effective QIF-NFM. 
In Laing's work---like in other recent related papers on pulse-coupled
oscillators \citep{Pazo2014,OKeeffe2016,Chandra2017,Coombes2016,Byrne2017,Gallego2017}--- 
...
nonlinear stability of the spatially homogeneous states of the QIF-NFM.

This analysis reveals  two important features: 
(i) The frequency of each oscillation mode only depends 
on the corresponding Fourier coefficient of the synaptic pattern of 
connectivity, and (ii) the decay rate is exactly the same for all
...
QIF-NFM [Eq.~\eqref{freG}] considering different membrane time constants $\tau$,
(or different main currents $\bar \eta$) for excitatory and inhibitory neurons.
As proved recently~\citep{Avitabile2017}, NFMs with time-scale separation display a rich
variety of robust spatiotemporal patterns, which may also be supported by our
model.
Also, recent work has been done to extend the local firing rate 
equations derived in~\citep{Montbrio2015} to include fixed synaptic delays~\citep{Pazo2016}
and synaptic kinetics \citep{Ratas2016,Devalle2017a}---see also
\citep{Coombes2016,Byrne2017}.
This work shows that time delays due to synaptic processing generally
lead to the emergence of self-sustained oscillations due to collective
synchronization. Extending the QIF-NFM [Eq.~\eqref{freG}]
to account for the synaptic time delays caused by synaptic processing 
may lead to spatiotemporal phenomena not previously observed 
in traditional NFMs.

\subsection*{\bf \sffamily Acknowledgments}

J.M.E.-A. and E.M. acknowledge support by European Union's Horizon
2020 research and innovation programme under the Marie
Sk\l{}odowska-Curie Grant No. 642563. J.M.E.-A. and E.M. acknowledge
the projects grants from the Spanish Ministry of Economics and
Competitiveness, Grants No. PSI2016-75688-P and
No. PCIN-2015-127. A.R. acknowledge a project grant from the Spanish
ministry of Economics and Competitiveness, Grant No. BFU2012-33413. A.R. has
been partially funded by the CERCA progam of the Generalitat
de Catalunya. D.A. was partially supported by the EPSRC
Grant No. EP/P510993/1 (United Kingdom).
\newpage
%%%%%%%%%%%%%%%%%
...
Our derivation closely follows that of~\citep{Montbrio2015}, but it needs to
be extended to include the spatial dimension. 
Similar extensions from a single population of phase oscillators 
to a one-dimensional, spatially distributed network with nonlocal 
coupling have been done 
in~\citep{Laing2014,Lai15,Laing2016a,Laing2016,Lai09i,Ome13,Omelchenko2014,Kawamura2014}.

Considering the thermodynamic limit $N\to\infty$, we can drop the 
indexes in Eqs.~\eqref{qif1} and \eqref{qif2}
...
Eqs.~\eqref{qif1} and \eqref{qif2} and $S \left( \phi,t \right) =
S^e \left( \phi,t \right) + S^i \left( \phi,t \right)$ represents the
total synaptic activity. 
Next we invoke the Ott-Antonsen theory~\citep{Ott2008},  
by means of the Lorentzian Ansatz (LA) \citep{Montbrio2015}  
%  
\begin{equation} 
...
\eta-i\Delta$ to obtain the neural field equations [Eq.~\eqref{freG}]\footnote{
The derivation of the QIF-NFM Eq.~\eqref{freG} can be readily extended
to account for population of excitatory and inhibitory neurons of
different sizes, i.e. $N^e\neq N^i$. This can be always achieved by
rescaling the relative contributions of the excitatory and inhibitory
mean fields as $S \left(\phi \right) = p^eS^e \left( \phi \right) +
p^i S^i \left( \phi \right)$, where $p^e = 2 N^e/\left( N^e + N^i
\right)$ and $p^i = 2N^i/\left( N^e + N^i \right)$. In the case of the
effective model Eq.~\eqref{fre}, this implies that Fourier
coefficients in Eq.~\eqref{mf} need to be changed as $J_K = p^eJ_K^e- p^iJ_K^i$.}   

These equations can be nondimensionalized  
...
%  
\begin{equation} 
  R^{e,i}= \frac{\sqrt{\Delta}}{\tau}  r^{e,i}\ , ~V^{e,i}
  =\sqrt{\Delta} ~ v^{e,i}\ ,
...
\end{subequations} 
%  
where the overdot represents derivation with respect the
nondimensional time $\tilde t$ and 
the mean field is 
%  
\begin{equation}
\begin{split}
...
%%%%%%%%%%%%%%%%%  
\subsection[Linear stability of  the
effective~QIF-NFM~Eq.~\eqref{fre}]{\sloppy Linear stability of  the\\
  effective~QIF-NFM~Eq.~\eqref{fre}} 
%%%%%%%%%%%%%%%%%  
...
\end{equation} 
%  
In terms of the dimensional variables and parameters \eqref{eq:AdimV} and \eqref{eq:AdimP},  
the eigenvalues \eqref{eq:mu0} are $\lambda_k t = \mu_k
\tilde{t}$, and thus $\lambda_k =\sqrt{\Delta}\mu_k/\tau$, which gives
the eigenvalues Eq.~\eqref{lambda} in the main text. 
...
\begin{equation} 
  ...
  \begin{pmatrix}  
    ...
    \sqrt{\frac{j_K^e-j_K^i }{2r_{*} }- \pi^2} 
  \end{pmatrix}. 
  \label{eq:eigenvect-perpendicular} 
\end{equation} 
%
have eigenvalues   
%  
...
the standing waves shown in Fig.~\ref{Figure2}. Additionally, the eigenvector
%  
\begin{equation} 
  \mathbf{u}_{K\perp}= 
  \begin{pmatrix}  
    ...
    \pi j_K^e 
  \end{pmatrix}, 
  \label{eq:eigenvect-orthogonal} 
\end{equation} 
%  
and its complex conjugate $\mathbf{\bar u}_{K\perp}$, with the associated
eigenvalue  
%  
\begin{equation} 
...
solutions    
using multiple-scale analysis, \citep[see~e.g.]{Bezruchko2010}.  
...
Accordingly, the (nondimensionalized) connectivity footprint
\eqref{J} is 
%  
\begin{equation} 
...
The first term on the right-hand side  of Eq.~\eqref{eq:O3eq} vanishes since  
...
\end{equation} 
%  
Finally, the nonlinear forcing term at the left-hand side of Eq.~\eqref{eq:O3eq} is  
%  
  \begin{eqnarray} 
  ...
  \eqref{eq:r20},~\eqref{eq:v20},~\eqref{eq:r22}, and \eqref{eq:v22}
  ...
  $j_2^c$  
  separating subcritical and supercritical Turing bifurcations: 
  %  
  \begin{equation} 
...
In dimensional form, Eqs.~\eqref{eq:AE}, \eqref{eq:b}, and \eqref{eq:J2c} are respectively:
%
\begin{equation} 
...
\end{algorithmic}
\end{algorithm}
\caption[Euler integration of QIF neurons.]{\label{alg:qif}
...
divided the ring into $m=100$ intervals located at $\phi_l =2\pi
l/m-\pi$, $l = 1, \dots, m$. 
At each interval $\phi_l$, we considered   $n = 2.5\cdot
10^3$ excitatory and $n=2.5\cdot 10^3$ inhibitory neurons (i.e. the
ring consisted of a total of $2N = 2m\ n = 5\times 10^5$ QIF
neurons).

The neurons in each location $\phi_l$ receive Lorentzian-distributed
currents, which have been generated using the formula
\begin{equation}
\label{eq:1}
\eta_i = \bar \eta + \Delta \tan \left[ \frac{\pi}{2} \frac{2i - n -
  1}{n + 1} \right],\ i = 1, \dots, n.
\end{equation}


On the other hand, perturbations (applied at time $t_0$) are modeled
using the function 
\begin{equation}
...
\end{equation}
where $A$ is the amplitude, $K$ is the wave number, and $\tau_r$ is
the rising time constant of the perturbation. In Figs.~\ref{Figure2},
\ref{Figure3} and \ref{Figure5} we used $t_0 = 0.05$~s, $A=0.3$, and
$\tau_r = 4\times 10^{-3}$~s. The
perturbations had a duration of $\Delta t = 0.01$ s. 

Finally, the instantaneous firing rates in Fig. \ref{Figure2} are obtained
binning time and counting the spikes of neurons in each interval $\phi_{l}$  within a sliding time window of size
$\delta t = 0.01$s. 
